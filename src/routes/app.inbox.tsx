...
function InboxPage() {
  ...
  const filtered = convos.filter((c: any) => !q || c.clients?.name?.toLowerCase().includes(q.toLowerCase()));
  const active = filtered.find((c: any) => c.id === selected) ?? filtered[0];
            ...
            filtered.map((c: any) => (
              ...
              >
                <div className="grid h-11 w-11 place-items-center rounded-full bg-primary/15 font-semibold text-primary">
                  {c.clients?.name?.split(" ").map((s: string) => s[0]).join("").slice(0, 2).toUpperCase() ?? "?"}
                </div>
            ...
            <header className="flex items-center justify-between border-b border-border bg-card px-5 py-3">
              <div className="flex items-center gap-3">
                <div className="grid h-10 w-10 place-items-center rounded-full bg-primary/15 font-semibold text-primary">
                  {active.clients?.name?.split(" ").map((s: string) => s[0]).join("").slice(0, 2).toUpperCase()}
                </div>
                <div>
...
function SampleBubble({ text, incoming }: { text: string; incoming?: boolean }) {
  return (
