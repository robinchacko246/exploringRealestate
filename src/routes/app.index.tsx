...
function StatCard({ icon: Icon, label, value, hint, accent }: { icon: any; label: string; value: string | number; hint?: string; accent?: boolean }) {
  return (
    <div className={`relative overflow-hidden rounded-xl border border-border bg-card p-5 shadow-[var(--shadow-soft)] ${accent ? "" : ""}`}>
  ...
  return (
    <div className="mx-auto max-w-7xl space-y-6 p-6">
      <div className="flex items-end justify-between">
        ...
        <Link to="/app/clients" className="text-xs font-medium text-primary hover:underline">
      ...
      <div className="grid gap-4 lg:grid-cols-3">
        <div className="rounded-xl border border-border bg-card p-5 lg:col-span-2">
          <div className="mb-4 flex items-center justify-between">
            ...
            <Link to="/app/clients" className="text-xs font-medium text-primary hover:underline">See all</Link>
          </div>
        ...
        <div className="rounded-xl border border-border bg-card p-5">
          <div className="mb-4 flex items-center justify-between">
            ...
            <Link to="/app/reminders" className="text-xs font-medium text-primary hover:underline">All</Link>
          </div>
...
function EmptyState({ icon: Icon, title, hint, cta, to }: any) {
  return (
    <div className="grid place-items-center py-10 text-center">
      ...
      {cta && to && (
        <Link to={to} className="mt-3 text-xs font-medium text-primary hover:underline">{cta}</Link>
      )}
    </div>
  );
}
