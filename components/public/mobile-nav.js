...
export default function MobileNav({ headerLinks, color = "#009688" }) {
  const [open, setOpen] = useState(false);

  const links = headerLinks && headerLinks.length > 0
    ? headerLinks
    : [
  ...
  return (
    <>
      ...
      {open && (
        <div
          className="absolute top-full left-0 right-0 bg-white border-b-2 shadow-lg md:hidden z-50"
          ...
          <nav className="max-w-[1280px] mx-auto px-6 py-4 flex flex-col gap-0">
            {links.map(({ href, label }) => (
              <Link
                ...
                className="text-[15px] text-[#424242] py-3 border-b border-[#F0F0F0] hover:opacity-80 transition-colors"
              >
                {label}
              </Link>
            ))}
            ...
            <Link
              href="/auth"
              onClick={() => setOpen(false)}
              className="mt-3 text-white text-[14px] font-semibold py-3 text-center rounded transition-colors"
